...
def signup():
    if request.method == 'POST':
        ...
        new_user = user_manager.signup(
            name, email, hash(password), dept, usergroup, regid)
...
def show_user(id=None):
    ...
    myreturnedbooks = user_manager.getReturnedBooksList(id)

    return render_template("profile.html", user=d, books=mybooks, returnedbooks=myreturnedbooks, g=g)
...
def update():
    ...
    user_manager.update(name, email, hash(password), bio,
                        dept, usergroup, regid, user_manager.user.uid())
